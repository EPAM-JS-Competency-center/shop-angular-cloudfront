...
if [ ! -z $config ]; then
    ...
    # result=$(cat <<< $result | jq '.pipeline.stages'  $filePath | jq 'map(.actions)' | jq --arg config $config 'map(map(. * {"EnvironmentVariables" : {"BUILD_CONFIGURATION" : $config}}))')
fi

echo $result | jq > pipeline-$updatedFileName.json
